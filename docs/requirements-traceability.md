...
| FR-OC-3 multi-asset declaration | Complete | CKB, UDT, and RGB++ schema plus Fiber currency mapping |
...
| FR-OR-3 reusable sessions | Complete | Repeated resolution, idempotency, and live two-node E2E script |
...
| FR-PY-2 confirmation propagation | Complete | Background `get_invoice` polling, status history, webhooks, receipts, reconciliation |
| FR-RC-1 recurrence declaration | Complete | Interval, amount, cycle cap, spending cap, and custom seconds |
| FR-RC-2 automatic triggering | Complete | Payer-owned `FiberRecurringPaymentScheduler` with due-cycle execution and retries |
...
- Live payment proof requires funded and connected external Fiber nodes. The repository includes guarded topology, route, and full E2E scripts; it cannot manufacture external testnet liquidity.
